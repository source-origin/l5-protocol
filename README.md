...
**Layout:** `src/` (9 contracts) · `test/` (13 suites / 112 tests) · `script/` (`DeployGovernance.s.sol` — 2-of-3 Safe + 48h timelock) · `lib/` (forge-std + openzeppelin-contracts, pinned as submodules).
...
Foundry suite — **112 tests, all passing** (`forge test`):
...
forge build --sizes
forge test -vvv                            # expect: 112 passed, 0 failed
```
