...
**Layout:** `src/` (9 contracts) · `test/` (14 suites / 122 tests) · `script/` (`DeployGovernance.s.sol` — 2-of-3 Safe + 48h timelock) · `lib/` (forge-std + openzeppelin-contracts, pinned as submodules).
...
Foundry suite — **122 tests, all passing** (`forge test`):
...
forge build --sizes
forge test -vvv                            # expect: 122 passed, 0 failed
```
